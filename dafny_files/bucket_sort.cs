// Dafny program bucket_sort.dfy compiled into C#
...
// Command-line arguments: run --no-verify /Users/rahinjain/Documents/GitHub/CS-472-Dafny-Python-Sorting-Algos-Verification/dafny_files/bucket_sort.dfy
// bucket_sort.dfy
...
method BucketSort(arr: array<int>) returns (new_arr: array<int>)
  ...
  ensures forall i: int, j: int {:trigger new_arr[j], new_arr[i]} :: 0 <= i < j < new_arr.Length ==> new_arr[i] <= new_arr[j]
  decreases arr
{
    ...
    decreases num_buckets - b
  {
      ...
      decreases |bucket| - j
    {
      ...
      while k < |sb| && bucket[j] > sb[k]
        invariant 0 <= k <= |sb|
        decreases |sb| - k, if k < |sb| then bucket[j] - sb[k] else 0 - 1
      {
        k := k + 1;
      }
...
method ArrayMaxNumber(arr: array<int>) returns (max_val: int)
  requires arr.Length > 0
  ensures forall i: int {:trigger arr[i]} :: 0 <= i < arr.Length ==> arr[i] <= max_val
  ensures exists i: int {:trigger arr[i]} :: 0 <= i < arr.Length && arr[i] == max_val
  decreases arr
{
  max_val := arr[0];
  var i := 1;
  while i < arr.Length
    ...
    invariant forall j: int {:trigger arr[j]} :: 0 <= j < i ==> arr[j] <= max_val
    invariant exists j: int {:trigger arr[j]} :: 0 <= j < i && arr[j] == max_val
    decreases arr.Length - i
  {
    if arr[i] > max_val {
      max_val := arr[i];
...
method ArrayMinNumber(arr: array<int>) returns (min_val: int)
  requires arr.Length > 0
  ensures forall i: int {:trigger arr[i]} :: 0 <= i < arr.Length ==> arr[i] >= min_val
  ensures exists i: int {:trigger arr[i]} :: 0 <= i < arr.Length && arr[i] == min_val
  decreases arr
{
  min_val := arr[0];
  var i := 1;
  while i < arr.Length
    invariant 1 <= i <= arr.Length
    invariant forall j: int {:trigger arr[j]} :: 0 <= j < i ==> arr[j] >= min_val
    invariant exists j: int {:trigger arr[j]} :: 0 <= j < i && arr[j] == min_val
    decreases arr.Length - i
  {
    if arr[i] < min_val {
      min_val := arr[i];
...
namespace _module {
  ...
  public partial class __default {
    public static void _Main(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      ...
      _0_arr = _nw0;
      (_0_arr)[(int)((BigInteger.Zero))] = new BigInteger(12);
      (_0_arr)[(int)((BigInteger.One))] = new BigInteger(5);
      (_0_arr)[(int)((new BigInteger(2)))] = new BigInteger(18);
      (_0_arr)[(int)((new BigInteger(3)))] = new BigInteger(11);
      (_0_arr)[(int)((new BigInteger(4)))] = new BigInteger(5);
      BigInteger[] _1_bucket__sort__test;
      BigInteger[] _out0;
      _out0 = __default.BucketSort(_0_arr);
      _1_bucket__sort__test = _out0;
      Dafny.Helpers.Print((_1_bucket__sort__test));
    }
    public static BigInteger[] BucketSort(BigInteger[] arr)
    {
      BigInteger[] new__arr = new BigInteger[0];
      BigInteger _0_min;
      BigInteger _out0;
      _out0 = __default.ArrayMinNumber(arr);
      _0_min = _out0;
      BigInteger _1_max;
      BigInteger _out1;
      _out1 = __default.ArrayMaxNumber(arr);
      _1_max = _out1;
      BigInteger _2_num__buckets;
      _2_num__buckets = new BigInteger((arr).Length);
      BigInteger _3_range;
      if (((_1_max) - (_0_min)).Sign == 0) {
        _3_range = BigInteger.One;
      } else {
        _3_range = (_1_max) - (_0_min);
      }
      Dafny.ISequence<BigInteger>[] _4_buckets;
      Dafny.ISequence<BigInteger>[] _nw0 = Dafny.ArrayHelpers.InitNewArray1<Dafny.ISequence<BigInteger>>(Dafny.Sequence<BigInteger>.Empty, Dafny.Helpers.ToIntChecked(_2_num__buckets, "array size exceeds memory limit"));
      _4_buckets = _nw0;
      BigInteger _5_i;
      _5_i = BigInteger.Zero;
      while ((_5_i) < (_2_num__buckets)) {
        (_4_buckets)[(int)((_5_i))] = Dafny.Sequence<BigInteger>.FromElements();
        _5_i = (_5_i) + (BigInteger.One);
      }
      _5_i = BigInteger.Zero;
      while ((_5_i) < (new BigInteger((arr).Length))) {
        BigInteger _6_index;
        _6_index = Dafny.Helpers.EuclideanDivision((((arr)[(int)(_5_i)]) - (_0_min)) * ((_2_num__buckets) - (BigInteger.One)), _3_range);
        (_4_buckets)[(int)((_6_index))] = Dafny.Sequence<BigInteger>.Concat((_4_buckets)[(int)(_6_index)], Dafny.Sequence<BigInteger>.FromElements((arr)[(int)(_5_i)]));
        _5_i = (_5_i) + (BigInteger.One);
      }
      Dafny.ISequence<BigInteger> _7_sorted;
      _7_sorted = Dafny.Sequence<BigInteger>.FromElements();
      BigInteger _8_total;
      _8_total = BigInteger.Zero;
      BigInteger _9_b;
      _9_b = BigInteger.Zero;
      while ((_9_b) < (_2_num__buckets)) {
        Dafny.ISequence<BigInteger> _10_bucket;
        _10_bucket = (_4_buckets)[(int)(_9_b)];
        Dafny.ISequence<BigInteger> _11_sb;
        _11_sb = Dafny.Sequence<BigInteger>.FromElements();
        BigInteger _12_j;
        _12_j = BigInteger.Zero;
        while ((_12_j) < (new BigInteger((_10_bucket).Count))) {
          BigInteger _13_k;
          _13_k = BigInteger.Zero;
          while (((_13_k) < (new BigInteger((_11_sb).Count))) && (((_10_bucket).Select(_12_j)) > ((_11_sb).Select(_13_k)))) {
            _13_k = (_13_k) + (BigInteger.One);
          }
          _11_sb = Dafny.Sequence<BigInteger>.Concat(Dafny.Sequence<BigInteger>.Concat((_11_sb).Take(_13_k), Dafny.Sequence<BigInteger>.FromElements((_10_bucket).Select(_12_j))), (_11_sb).Drop(_13_k));
          _12_j = (_12_j) + (BigInteger.One);
        }
        _7_sorted = Dafny.Sequence<BigInteger>.Concat(_7_sorted, _11_sb);
        _8_total = (_8_total) + (new BigInteger((_11_sb).Count));
        _9_b = (_9_b) + (BigInteger.One);
      }
      BigInteger[] _nw1 = new BigInteger[Dafny.Helpers.ToIntChecked(new BigInteger((arr).Length), "array size exceeds memory limit")];
      new__arr = _nw1;
      BigInteger _14_m;
      _14_m = BigInteger.Zero;
      while ((_14_m) < (new BigInteger((arr).Length))) {
        (new__arr)[(int)((_14_m))] = (_7_sorted).Select(_14_m);
        _14_m = (_14_m) + (BigInteger.One);
      }
      new__arr = new__arr;
      return new__arr;
      return new__arr;
    }
    public static Dafny.ISequence<Dafny.ISequence<BigInteger>> CreateEmptyBuckets(BigInteger n)
    {
      Dafny.ISequence<Dafny.ISequence<BigInteger>> result = Dafny.Sequence<Dafny.ISequence<BigInteger>>.Empty;
      result = Dafny.Sequence<Dafny.ISequence<BigInteger>>.FromElements();
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      while ((_0_i) < (n)) {
        result = Dafny.Sequence<Dafny.ISequence<BigInteger>>.Concat(result, Dafny.Sequence<Dafny.ISequence<BigInteger>>.FromElements(Dafny.Sequence<BigInteger>.FromElements()));
    ...
    public static BigInteger ArrayMaxNumber(BigInteger[] arr)
    {
      BigInteger max__val = BigInteger.Zero;
      max__val = (arr)[(int)(BigInteger.Zero)];
      BigInteger _0_i;
      _0_i = BigInteger.One;
      while ((_0_i) < (new BigInteger((arr).Length))) {
        if (((arr)[(int)(_0_i)]) > (max__val)) {
          max__val = (arr)[(int)(_0_i)];
        }
        _0_i = (_0_i) + (BigInteger.One);
      }
      max__val = max__val;
    ...
    public static BigInteger ArrayMinNumber(BigInteger[] arr)
    {
      BigInteger min__val = BigInteger.Zero;
      min__val = (arr)[(int)(BigInteger.Zero)];
      BigInteger _0_i;
      _0_i = BigInteger.One;
      while ((_0_i) < (new BigInteger((arr).Length))) {
        if (((arr)[(int)(_0_i)]) < (min__val)) {
          min__val = (arr)[(int)(_0_i)];
        }
        _0_i = (_0_i) + (BigInteger.One);
      }
      min__val = min__val;
