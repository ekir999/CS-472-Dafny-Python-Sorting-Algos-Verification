  ...
  ensures forall i, j :: 0 <= i < j < new_arr.Length ==> new_arr[i] <= new_arr[j]
{
  ...
  while i < arr.Length
    ...
    //check for distribution of elements into buckets
  {
    var index := ((arr[i] - min) * (num_buckets - 1)) / range;
    assert 0 <= index < num_buckets; // index should be within bounds
  ...
  var total := 0;
  var b := 0;
  //sum of all buckets should be equal to the length of the array
  while b < num_buckets
    invariant 0 <= b <= num_buckets
    //invariant total <= arr.Length
    invariant |sorted| == total
    //invariant total == (sum i | 0 <= i < :: buckets[i].Length);
    decreases num_buckets - b
  {
      ...
      invariant |sb| == j
    {
      ...
      while k < |sb| && bucket[j] > sb[k]
        invariant 0 <= k <= |sb|
        invariant forall m :: 0 <= m < k ==> sb[m] <= bucket[j]
      {
        k := k + 1;
      }
